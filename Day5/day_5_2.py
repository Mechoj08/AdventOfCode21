...
def horizontal_vertical_diagonal(coordinate_pairs):
    new_pairs = []
    for i in coordinate_pairs:
        diff = i[1]-i[0]
        diag_diff = abs(diff.real)-abs(diff.imag)
        if diff.real == 0 or diff.imag == 0 or diag_diff == 0:
            new_pairs.append(i)
    return new_pairs
...
def covered_coordinates(pair):
    ...
    if diff.real == 0:
        ...
        if diff.real > 0 and diff.imag > 0:
            for i in range(int(abs(diff.real))+1):
                covered.append(start+complex(i, i))
        elif diff.real > 0 and diff.imag < 0:
            for i in range(int(abs(diff.real))+1):
                covered.append(start+complex(i, -i))
        elif diff.real < 0 and  diff.imag > 0:
            for i in range(int(abs(diff.real))+1):
...
def puzzle_1():
    ...
    left_over_pairs = horizontal_vertical_diagonal(original_pairs)
    all_coordinates = []
